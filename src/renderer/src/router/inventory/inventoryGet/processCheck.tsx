...
const HQPage = (): JSX.Element => {
  ...
  const OceanListGet = async (): Promise<void> => {
    ...
    const vendorresult = [
      { value: '大洋商会', label: '大洋商会' },
      { value: '大洋以外', label: '大洋以外' }
    ]
  ...
  const PrintProcessList = async (): Promise<void> => {
    try {
      ...
      const storeOrders = storeList.map((item) => {
        ...
        const resultdata = { storeName: item, process: processdata }
        return resultdata
      })
  ...
  const VendorPrint = async (): Promise<void> => {
  ...
  const VendorOrderData = async (): Promise<void> => {
  ...
  const casePrint = async (): Promise<void> => {
    const setDate = new Date(getDate).toLocaleDateString()
    const stores = checkresult.filter(
      (item: CheckResultItem) =>
        item.process !== '未注文' && item.process !== '印刷済' && item.process !== '注文無'
    )
  ...
  const IndividualPrint = async (): Promise<void> => {
    const setDate = new Date(getDate).toLocaleDateString()
    if (!storeSelect) {
      toast.error('印刷できるデータがありません')
      return
  ...
  const handleallPrint = async (): Promise<void> => {
    const setDate = new Date(getDate).toLocaleDateString()
      ...
      <div className="chack_etc">
        <div className="check_area">
          <div className="check_set">
            <Button variant="outlined" onClick={() => PrintProcessList()}>
              取得
            </Button>
            ...
            <div className="print-set-area">
              ...
              <div className="print-select-area">
                <div className="order-print">
                  ...
                  <Button variant="outlined" onClick={IndividualPrint}>
                    個別印刷
                  </Button>
                  <Button variant="outlined" onClick={casePrint}>
                    全未印刷
                  </Button>
                  <Button variant="outlined" onClick={handleallPrint}>
                    全印刷
                  </Button>
                </div>
                <div className="order-print-vendor">
                  ...
                  <Button variant="outlined" onClick={VendorOrderData}>
                    取得
                  </Button>
                  <Button variant="outlined" onClick={VendorPrint}>
                    印刷
                  </Button>
